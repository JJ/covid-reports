...
```{r logistic}
data.3.6$cap <- rep(45000000,9)
model.logistic <- prophet(data.3.6, growth='logistic')
future.logistic <- make_future_dataframe(model.logistic, periods=3)
forecast.logistic <- predict(model.logistic,future.logistic)
forecast.logistic$ds <- as.Date(forecast.logistic$ds, "%Y-%m-%d")
ggplot(forecast.logistic,aes(x=ds)) + geom_point(data=forecast.logistic,aes(y=yhat,color="Predicted"))+ geom_point(data=forecast.logistic,aes(y=yhat_upper,color="Upper limit")) + geom_point(data=data.3.6,aes(y=y,color="Real")) + theme_economist()
