...
ggplot(world.data.filtered.1k, aes(x=Date,y=CFR,color=Territory,group=Territory))+geom_line()+theme_tufte()+theme(legend.position = "bottom")+labs(title = "Evolution of case fatality ratio",subtitle = "Countries and territories with more than 1k cases")
```
...
This file has been generated from data published by [JHU CSSE](https://github.com/CSSEGISandData/COVID-19). It's data-driven and it can be re-generated from the script in  [this repository](https://github.com/JJ/covid-reports). This report uses RMarkdown and it can be downloaded from [here](https://github.com/JJ/covid-reports/blob/master/cfr-evolution.Rmd).
