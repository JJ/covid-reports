...
data <- data %>% mutate( R0= total / lag(total, 1, default=1))
data <- data %>% mutate( HR0= h.total / lag(h.total, 1, default=1))
...
Se habla de `R0` como la relación de personas infectadas por cada una que tenga COVID-19. La estimación directa es imposible, pero de forma indirecta se puede estimar a partir del número total de casos no fallecidos ni dados de alta, comparándolo con el del día anterior.

```{r R0}
ggplot(data,aes(x=Fecha,y=R0))+geom_point()+geom_line()+theme_economist()
```

Siendo la cantidad de hospitalizados la más fiable, conviene calcularlo usándola, aunque todavía hay pocos datos:
