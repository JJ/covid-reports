...
```{r CFR,warning=FALSE}
ggplot(data,aes(x=Fecha,y=CFR,color='red'))+geom_line()+geom_segment(aes(xend=max(Fecha), yend = CFR), linetype=2, colour='pink')+theme_economist()+
  geom_text(aes(x = max(Fecha)+.1, label = sprintf("%.2f", CFR*100), hjust=0))
```
